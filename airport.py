...
class Airport:
    ...
    def get_icao(self):
        """
        Prompts the user for the departure airport code. It must be in the supported list
        """
        while True:
    ...
    def specific_airport(self, route, aircraft_type: AircraftType):
        """
        Checks which departure airport
        Then it uses the airport's specific methods to get the initial altitude, departure procedure, and climb for the aircraft
        Some procedures depend on the aircraft's route and/or aircraft type, as well as the operations at that airport or those around it
        """
        if self.icao == "KEWR":
    ...
    def get_dep_freq(self):
        """Returns the departure frequency in the specific airport's .py file"""
        if self.icao == "KEWR":
            return ewr.DEP_FREQ
        if self.icao == "KJFK":
            return jfk.DEP_FREQ
    ...
    def get_exit_gates(self):
        """Returns all the airport's departure gates in the specific airport's .py file"""
        if self.icao == "KEWR":
            return ewr.ALL
        if self.icao == "KJFK":
            return jfk.ALL
